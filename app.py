...
EXTENDED_LOOKAHEAD_LEAGUES = {
    path for path in ESPN_API_PATHS
    if path.startswith(('soccer/', 'football/', 'volleyball/'))
    and path not in MOTORSPORTS_LEAGUES
} | GOLF_LEAGUES
COLLEGE_LEAGUES = {
...
def _refresh_espn_cache():
    ...
    for path in leagues:
        try:
            refreshed[path] = _fetch_league_raw(path, lc)
        except Exception as e:
...
def _build_data_response(active_leagues, active_teams, active_lc, tz_str, settings):
    ...
    try:
        ...
        fav_min_date = today_local if local_now.hour >= 11 else (today_local - timedelta(days=1))
    except Exception:
        today_local = now.date()
        fav_min_date = today_local
