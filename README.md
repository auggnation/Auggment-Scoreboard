That’s a great name—Auggmented Scoreboard fits perfectly, especially considering your nickname and the way this project "augments" a standard TV into something much more functional.

Below is the updated README tailored for your project.
...
Racing & Golf: Specialized leaderboard cards for NASCAR, IndyCar, F1, and PGA.  


Local Sports Integration: Automatically parses school calendars (Thrillshare, MaxPreps, iCal) to show local high school or community games.  


Kiosk/Server Architecture: Run one central Server and pair multiple Kiosks (TVs) to it for synchronized data but independent display settings.  
...
Environment: The installer is designed for a non-graphical (CLI) environment or a fresh OS install. It will install and configure its own minimal window manager (Openbox) and display server (X11) for Kiosk mode.

🛠️ Installation
Prepare the OS: Start with a fresh install of Raspberry Pi OS or Ubuntu/Debian.

Download Files: Place app.py, install.sh, version.txt, and the templates/ folder in a directory (e.g., /home/auggie/scoreboard).
...
Fallback: If Chrome is not found or fails, the install.sh script will automatically fall back to chromium-browser.
