...
# ─── SUDOERS — all modes (app.py calls sudo systemctl to control kiosk.service)
echo "--- Configuring Sudoers ---"
SYSTEMCTL_PATH=$(command -v systemctl)
cat > /etc/sudoers.d/scoreboard <<EOF
# Allow the scoreboard user to control the kiosk service without a password
...
if [[ "$MODE" == "server" ]]; then
    ...
    for f in "$APP_DIR/app.py" \
              ...
              "$APP_DIR/templates/settings.html"; do
